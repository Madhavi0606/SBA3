...
public class UserServiceImpl implements UserService {
	...
	public List<CustAcctModel> getFixedDepositInfo(List<CustAcctModel> custAcct1) {
		List<CustAcctModel> custAcct = custAcct1;
		for(int i =0;i<custAcct.size();i++) {
			if((custAcct.get(i).getCustAcctType()).equalsIgnoreCase("Recurring Deposit")) {
				custAcct.remove(i);
	...
	public List<CustAcctModel> getRecurringDepositInfo(List<CustAcctModel> custAcct1) {
		List<CustAcctModel> custAcct = custAcct1;
		for(int i =0;i<custAcct.size();i++) {
			if((custAcct.get(i).getCustAcctType()).equalsIgnoreCase("Savings Account")) {
				custAcct.remove(i);
	...
	public AcctTransactionModel transferFunds(@Valid AcctTransactionModel transferObj) throws BankException{
			...
			double existingBal = getSavingsAcctData(customerRepo.findByCustAcctNum(transferObj.getTransFrom())).getAvailableBalance();
	...
	public CustAcctModel updateCustBal(AcctTransactionModel txnObj) {
		List<CustAcctEntity> custAcct1 = customerRepo.findByCustAcctNum(txnObj.getTransFrom());
	...
	private AcctTransactionEntity txnParse(@Valid AcctTransactionModel source) {
		...
		target.setCustAcctNum(source.getT());
	...
	public List<AcctTransactionModel> getFilteredStatement(AccountStatementModel filterStmtData) throws ParseException {
		...
		List<AcctTransactionModel> txnObj = custTxnRepo.findByCustAcctNum(filterStmtData.getCustAcctNum()).stream().map(e -> custTxnParse(e)).collect(Collectors.toList());
